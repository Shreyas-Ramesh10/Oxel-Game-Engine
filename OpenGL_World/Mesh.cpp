...
void Mesh::CreateMesh(GLfloat* vertices, unsigned int* indices, unsigned int numOfVertices, unsigned int numOfIndices) 
{
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(vertices[0]) * 8, 0); //1st value is the index of the vertex attribute(Location), 2nd value is the number of components in the vertex attribute, 3rd value is the type of data, 4th value is whether to normalize the data or not, 5th value is the stride (0 means tightly packed), 6th value is the offset (0 means start from beginning)
